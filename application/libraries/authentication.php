	...
	class Authentication
	{
		...
		function process_admin_login ($login = NULL, $request = '')
		{
				...
				if (0 < $select_query->num_rows ())
				{
                                    ...
                                    $this->CI->session->set_userdata ('admin_logged_in',$session_data);
                                    return 'success';
				}
				else 
                                    return false;
	   ...
	   function process_user_login ($login = NULL, $request = '')
		{
    			...
    			$password        = $login['password'];
				$this->CI->db->select ('username   AS USERNAME, password, CONCAT_WS(" ", first_name, last_name) as NAME, id as USERID, email as EMAIL,company_id as COMPANY', false);
		...
		function process_user_auto_login ($login = NULL, $request = '')
		{
    			if (!is_array ($login) || 0 >= count ($login))
    			{
    				return FALSE;
