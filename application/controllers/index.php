...
class Index extends CI_Controller {
	...
	public function index($company_url='') {
		
	   if($company_url!='' and $this->common_model->check_company_url($company_url)){
		   if($this->common_model->check_company_access($company_url)){	  
			 if($this->session->userdata('logged_in')){
				 ...
				 $this->gen_contents['company_url']=$company_url;
				 $this->gen_contents['auth_code']=(isset($_GET['auth']) ? $_GET['auth'] : '' ); 
				 ...
				 $this->gen_contents['company_page_graphics']=$this->common_model->get_company_page_graphics($this->gen_contents['company_id']);
				 $this->gen_contents['company_page_slider']=$this->common_model->get_data('company_page_sliders','*',array("company_id"=>$this->gen_contents['company_id'],"status"=>1));
				 $this->gen_contents['company_custom_menu']=$this->common_model->get_data('company_custom_menu','*',array("company_id"=>$this->gen_contents['company_id'],"status"=>1));
				 ...
				 $this->gen_contents['onair_track_protocol_type']=$this->common_model->get_company_onair_programmme($this->gen_contents['company_id'],'protocol_type');
			 ...
			 if(isset($_GET['auth'])){   
			   ...
			   if($user_details){   
				   ...
				   $this->load->view('common/main_header',$this->gen_contents);
				   redirect(''.$company_url.'?auth='.$_GET['auth'].'', 'refresh');				  
			   }
			   else{
				   				
				 $this->gen_contents['current_controller'] = "Index";        
				 $this->gen_contents['page_title'] = "Corporate Radio";         
				 $this->load->view('common/login_header',$this->gen_contents);
				 $this->load->view('site/404',$this->gen_contents);
				 $this->load->view('common/footer',$this->gen_contents);       			   
			   }
		     ...
		     $this->gen_contents['current_controller'] = "Index";        
		   	 $this->gen_contents['page_title'] = "Unauthorized Access";         
			 $this->load->view('common/login_header',$this->gen_contents);
			 $this->load->view('site/no_access',$this->gen_contents);
			 $this->load->view('common/footer',$this->gen_contents);    
  ...
  public function logout($company_url) {
